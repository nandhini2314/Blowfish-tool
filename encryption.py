...
def binToHex(binary_string):
    ...
    for i in range(0, len(binary_string), 4):
        ...
        hex_string += hex_mapping[four_bits]
...
def f(plainText):
    ...
    for i in range(0,8,2):
        substring = plainText[i:i + 2]
        col = int(substring, 16)
...
def key_generate(key):
    ...
    for i in range(len(P)):
        ...
        print(f"subkey {i + 1}: {P[i]}")
        j = (j + 8) % len(key)

def round_function(time, plain_text):
    left = plain_text[:8]
    right = plain_text[8:]
    left = XOR(left, P[time])
    f_out = f(left)
    right = XOR(f_out, right)

    print(f"round {time}: {right}{left}")
    #returning like this basically swaps the positions, simulating the Fiestel cipher
    return right + left


def encrypt(plainText):
    for i in range(16):
        plainText = round_function(i,plainText)
    #rounds finished
    #then
    right = plainText[:8]
    left = plainText[8:]
